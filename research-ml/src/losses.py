...
class LogitAdjustedCELoss(nn.Module):
    def __init__(self, class_counts: torch.Tensor, tau: float) -> None:
        super().__init__()
    ...
    def forward(self, logits: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        return F.cross_entropy(logits + self.tau * self.log_priors.to(logits.device), target)
...
def build_loss(config: dict[str, Any], class_counts: list[int], device: torch.device) -> nn.Module:
    ...
    if float(imbalance.get("logit_adjustment_tau", 0.0)) > 0 or imbalance["loss"] == "logit_adjustment":
        ...
        return LogitAdjustedCELoss(counts, tau=tau).to(device)
    weights = class_weight_tensor(imbalance.get("class_weights", "none"), counts)
    if weights is not None:
        weights = weights.to(device)
    if imbalance["loss"] == "focal":
        return FocalLoss(gamma=float(imbalance["focal_gamma"]), weight=weights).to(device)
