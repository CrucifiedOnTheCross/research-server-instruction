...
NAME="${NAME:-longtail-stage3a-screening}"
...
docker run -d \
  ...
  "$IMAGE" \
  bash -lc "source '$RESEARCH_VENV/bin/activate' && set -o pipefail && python tools/run_stage3a_training_matrix.py && python tools/run_stage3a_logit_adjustment.py && python tools/check_stage3a_readiness.py 2>&1 | tee 'server-logs/stage3a_$STAMP.log'"

echo "Container: $NAME"
echo "Structured results: $WORKDIR/outputs/stage3a_screening"
