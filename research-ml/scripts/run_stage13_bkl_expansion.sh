...
python tools/generate_synthetic_img2img.py \
  --config configs/stage13_generation_bkl_expansion.yaml \
...
python tools/select_stage13_multiencoder_coverage.py \
  ...
  --additional-pool-csv synthetic/ham10000_stage13_bkl_expansion_img2img/synthetic_manifest.csv \
  --strict-csv splits/stage10/selected_synthetic_strict_id.csv \
  --out-dir outputs/reports/stage13_bkl_expansion_selection \
  --split-out-dir splits/stage13_bkl_expansion \
  --expected-pool-size 1120
