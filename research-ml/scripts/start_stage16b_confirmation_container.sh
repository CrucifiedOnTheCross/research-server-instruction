...
NAME="${NAME:-research-stage16b-confirmation}"
...
docker run -d \
  ...
  "$IMAGE" \
  bash -lc "source .venv/bin/activate && set -o pipefail && bash scripts/run_stage16b_confirmation.sh 2>&1 | tee 'server-logs/stage16b_confirmation_$STAMP.log'"

echo "Container: $NAME"
echo "Structured results: $WORKDIR/outputs/stage16_isic2019_*"
echo "MLflow: http://10.200.1.180:5000/#/experiments/5"
