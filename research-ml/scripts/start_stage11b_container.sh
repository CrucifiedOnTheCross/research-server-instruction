...
NAME="${NAME:-research-stage11b-confirmatory}"
...
docker run --rm \
  ...
  -w "$WORKDIR" \
  "$IMAGE" \
  bash -lc "source .venv/bin/activate && python tools/check_stage11b_gate.py"
...
docker run -d \
  ...
  "$IMAGE" \
  bash -lc "source .venv/bin/activate && set -o pipefail && bash scripts/run_stage11b_confirmatory.sh 2>&1 | tee 'server-logs/stage11b_$STAMP.log'"

echo "Container: $NAME"
echo "Structured results: $WORKDIR/outputs/stage11b_*"
