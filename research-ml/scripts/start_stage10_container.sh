...
NAME="${NAME:-research-stage10-geometry}"
...
docker run -d \
  ...
  "$IMAGE" \
  bash -lc "source .venv/bin/activate && set -o pipefail && bash scripts/run_stage10_geometry_strata.sh 2>&1 | tee 'server-logs/stage10_$STAMP.log'"

echo "Container: $NAME"
echo "Structured results: $WORKDIR/outputs/stage10_*"
echo "Summary: $WORKDIR/outputs/reports/stage10_multiseed"
