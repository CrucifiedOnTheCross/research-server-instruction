...
docker rm -f research-stage13-bkl-expansion >/dev/null 2>&1 || true
docker run -d \
  --name research-stage13-bkl-expansion \
  --shm-size=24g \
  --gpus all \
  --user "$USER_SPEC" \
  --group-add "$GROUP_ID" \
  ...
  bash -lc "source .venv/bin/activate && set -o pipefail && bash scripts/run_stage13_bkl_expansion.sh 2>&1 | tee server-logs/stage13_bkl_expansion_$(date +%Y%m%d-%H%M%S).log"
