...
docker rm -f research-stage16g-p1-domain-lora >/dev/null 2>&1 || true
docker run -d \
  --name research-stage16g-p1-domain-lora \
  ...
  bash -lc "set -o pipefail && bash scripts/run_stage16g_p1_domain_lora.sh 2>&1 | tee server-logs/stage16g_p1_$(date +%Y%m%d-%H%M%S).log"
