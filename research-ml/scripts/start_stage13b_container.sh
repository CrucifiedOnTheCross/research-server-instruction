...
NAME="${NAME:-research-stage13b-confirmatory}"
...
docker run --rm \
  ...
  -w "$WORKDIR" \
  "$IMAGE" \
  bash -lc "source .venv/bin/activate && python tools/check_stage13b_gate.py"
...
docker run -d \
  ...
  "$IMAGE" \
  bash -lc "source .venv/bin/activate && set -o pipefail && bash scripts/run_stage13b_confirmatory.sh 2>&1 | tee 'server-logs/stage13b_$STAMP.log'"

echo "Container: $NAME"
echo "Structured results: $WORKDIR/outputs/stage13b_*"
